...
class GridWorld:
    ...
    goal_states : numpy arrany of shape (n, 2)
        ...
        states.
    """
    def __init__(self, num_rows, num_cols, start_state, small_goal_states, big_goal_states):
        self.num_rows = num_rows
        self.num_cols = num_cols
        self.start_state = start_state
        self.small_goal_states = small_goal_states
        self.big_goal_states = big_goal_states
        ...
        self.bias = None
        self.r_step = None
        self.r_smallgoal = None
        self.r_biggoal = None
        self.r_dead = None
        self.r_wallbump = None
        self.gamma = 1 # default is no discounting

    def add_obstructions(self, obstructed_states=None, bad_states=None, restart_states=None, surprise_states=None):
        """
        ...
        self.surprise_states = surprise_states
        if surprise_states is not None:
            self.num_surprise_states = surprise_states.shape[0]
        else:
            self.num_surprise_states = 0
    ...
    def add_rewards(self, step_reward, small_goal_reward, big_goal_reward, bad_state_reward=None, restart_state_reward = None, surprise_state_min_reward=None, surprise_state_max_reward=None, wallbump_reward = None):
        """
        ...
        self.r_smallgoal = small_goal_reward
        self.r_biggoal = big_goal_reward
        self.r_bad = bad_state_reward
        self.r_restart = restart_state_reward
        self.r_surprise_min = surprise_state_min_reward
        self.r_surprise_max = surprise_state_max_reward
        self.r_wallbump = wallbump_reward
    ...
    def create_gridworld(self):
        ...
        self.small_goal_states_seq = row_col_to_seq(self.small_goal_states, self.num_cols)
        self.big_goal_states_seq = row_col_to_seq(self.big_goal_states, self.num_cols)
        if self.restart_states is not None:
            self.restart_states_seq = row_col_to_seq(self.restart_states, self.num_cols)
        else:
            self.restart_states_seq = None
        # rewards structure
        self.surprise_tag = np.zeros((self.num_states, 1))
        self.R = self.r_step * np.ones((self.num_states, self.num_actions))
        self.R[self.num_states-1, :] = 0
        
        ## Add wall bump pain
        if self.r_wallbump is not None:
            for state in range(self.num_states):
                for action in range(self.num_actions):
                    direction = self._get_direction(action, direction=0)
                    if self._get_wallbump(state, direction):
                        self.R[state, action] = self.r_wallbump
        ...
        self.R[self.small_goal_states_seq, :] = self.r_smallgoal
        self.R[self.big_goal_states_seq, :] = self.r_biggoal
        # print(self.R)

        # tag the surprise states
        for i in range(self.num_surprise_states):
            surprise_state = row_col_to_seq(self.surprise_states[i,:].reshape(1,-1), self.num_cols)
            self.surprise_tag[surprise_state, :] = 1
        ...
        for action in range(self.num_actions):
            for state in range(self.num_states):
                ...
                if self.obs_states is not None:
                    end_states = np.vstack((self.obs_states, self.small_goal_states, self.big_goal_states))
                else:
                    end_states = np.vstack((self.small_goal_states, self.big_goal_states))
    ...
    def _get_wallbump(self, state, direction):
        """
