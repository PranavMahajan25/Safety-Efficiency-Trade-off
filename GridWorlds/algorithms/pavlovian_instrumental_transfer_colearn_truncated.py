...
def tdlearning(model, mode, pav_pain = True, w=0, modulate_w=False, random_restarts=True, alpha=0.1, gamma=1.0, mu = 50, tau_0=1.0, tau_k=0.0, maxiter=None, maxeps=2000, dynobs=False, goalchange=None, saverun_folder='./run_outputs/', run_num = 1, kappa = 2, beta = 0.5, fix_Pav_action=False):
    """
    ...
    fix_Pav_action : bool
        If true, then A_p = [4] (immobilizing action), else derived from Q_p
    ...
    ep_completion_failures = []
    cum_completion_failures = []
    w_list = []
    TDE_list = []
    ...
    ## Important - not using model.num_actions
    num_actions = 5
    Q = np.random.randn(model.num_states, num_actions) * init_sigma
    ...
    if pav_pain:
        Qp = np.random.randn(model.num_states, num_actions) * init_sigma
    else:
        Qp = None
    ...
    for i_episode in range(maxeps):
        ...
        while True:
            ...
            if modulate_w:
                ...
                # print(w)
            action,_ = sample_action(Q, (V,Vr,Vp), state, num_actions, tau, mu, Qp, w, fix_Pav_action)
            # print("Action:", action)
            ...
            if mode == 'sarsamax':
                ...
                next_action, _ = sample_action(Q, (V,Vr,Vp), next_state, num_actions, tau, mu, Qp, w, fix_Pav_action)
        ...
        ep_completion_failures.append(ep_completion_failure)
        cum_completion_failure += ep_completion_failure
        cum_completion_failures.append(cum_completion_failure)

        # Add trajectory visualizing script
        if visualise:
            if i_episode == 900:
                env_name = "fear_avoidance_env"
                # visualise_trajectory(env_name, i_episode, model, maxiter, dynobs, modulate_w, running_avg_absTDE, kappa, Q, V,Vr,Vp, tau, mu, Qp, w, num_actions)
    ...
    spi = np.zeros((model.num_states, num_actions))
    for state in range(model.num_states):
        if modulate_w:
            w = compute_w(state, running_avg_absTDE, kappa)
        _, action_probs = sample_action(Q, (V,Vr,Vp), state, num_actions, tau, mu, Qp, w, fix_Pav_action)
        spi[state, :] = action_probs
    pi = np.argmax(spi, axis=1).reshape(-1,1) # combined policy

    Qp_spi = np.zeros((model.num_states, num_actions))
    for state in range(model.num_states):
        _, action_probs = sample_action(Qp, (V,Vr,Vp), state, num_actions, tau, mu, None, w=0, fix_Pav_action=fix_Pav_action)
    ...
    if plot_plots:
        ...
        plt.ylabel('omega value (modulated)')
        plt.xlabel('Episode #')
        plt.show()

        # plot the completion failures
        fig = plt.figure()
        ax = fig.add_subplot(111)
        plt.plot(np.arange(len(cum_completion_failures)), cum_completion_failures)
        plt.ylabel('Cummulative episode completion failures')
        plt.xlabel('Episode #')
        plt.show()
    ...
    saverun_folder_cum_completion_failures = os.path.join(saverun_folder, 'cum_completion_failures')
    ...
    os.makedirs(saverun_folder_cum_completion_failures, exist_ok=True)
    ...
    with open(os.path.join(saverun_folder_cum_completion_failures, 'run_'+str(run_num)+'.npy'), 'wb') as f:
        np.save(f, np.array(cum_completion_failures))
...
def sample_action(Q, Vs, state, num_actions, tau, mu, Qp = None, w=0, fix_Pav_action=False):
    """
