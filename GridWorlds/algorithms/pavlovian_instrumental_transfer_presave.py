...
from run_codes.dynamic_environment import move_goal


def tdlearning(model, mode, pavlovian_reward_qvalues=None, pavlovian_pain_qvalues=None, w=0, modulate_w=False, random_restarts=True, alpha=0.1, gamma=1.0, mu = 50, tau_0=1.0, tau_k=0.0, maxiter=None, maxeps=2000, goalchange=None, saverun_folder='./run_outputs/', run_num = 1, kappa = 2, beta = 0.5, two_goals=False):
    ...
    pavlovian_reward_qvalues : numpy array
        Optimal policy trained using Pavlovian system

    pavlovian_pain_qvalues : numpy array
        Optimal policy trained using Pavlovian system
    ...
    for i_episode in range(maxeps):
        ...
        while True:
            ...
            if modulate_w:
                ...
                # print(w)
            action,_ = sample_action(Q, (V,Vr,Vp), state, model.num_actions, tau, mu, pavlovian_reward_qvalues, pavlovian_pain_qvalues, w)
            # print("Action:", action)
            ...
            if mode == 'sarsamax':
                ...
                next_action, _ = sample_action(Q, (V,Vr,Vp), next_state, model.num_actions, tau, mu, pavlovian_reward_qvalues, pavlovian_pain_qvalues, w)
    ...
    for state in range(model.num_states):
        if modulate_w:
            w = compute_w(state, running_avg_absTDE, kappa)
        _, action_probs = sample_action(Q, (V,Vr,Vp), state, model.num_actions, tau, mu, pavlovian_reward_qvalues, pavlovian_pain_qvalues, w)
    ...
    return (V,Vr,Vp), q, pi, state_counts, spi, absTDE
...
def sample_action(Q, Vs, state, num_actions, tau, mu, pavlovian_reward_qvalues=None, pavlovian_pain_qvalues=None, w=0):
    """
    ...
    if pavlovian_reward_qvalues is not None:
        A_r = np.argwhere(pavlovian_reward_qvalues[state, :] == np.amax(pavlovian_reward_qvalues[state, :]))
        A_r = A_r.flatten().tolist()
    if pavlovian_pain_qvalues is not None:
        A_p = np.argwhere(pavlovian_pain_qvalues[state, :] == np.amax(pavlovian_pain_qvalues[state, :]))
        A_p = A_p.flatten().tolist()
    
    if pavlovian_reward_qvalues is None and pavlovian_pain_qvalues is None:
        advantages = Q[state, :] - V[state]
    elif pavlovian_reward_qvalues is not None and pavlovian_pain_qvalues is None:
        advantages = (Q[state, :] - V[state]) * (1- w)
        for a_r in A_r:
            advantages[a_r] += Vr[state] * w
    elif pavlovian_reward_qvalues is None and pavlovian_pain_qvalues is not None:
        # print(Q[state, :], V[state], w)
        advantages = (Q[state, :] - V[state]) * (1- w)
        for a_p in A_p:
