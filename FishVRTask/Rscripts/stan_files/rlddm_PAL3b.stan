...
parameters {
  ...
  vector[N] omega_pr;       // arbitration parameter
}
...
transformed parameters {
  ...
  for (i in 1:N) {
    ...
    b[i] = Phi_approx(mu_pr[5] + sigma[5] * b_pr[i]);
  }
  threshold = exp(mu_pr[3] + sigma[3] * threshold_pr);
  omega = exp(mu_pr[6] + sigma[6] * omega_pr);
  ...
  for (i in 1:N) {
    ...
    for (t in 1:Tsubj[i]) {
      ...
      if (pressed[i, t]) { // drift to approach
        RT[i,t] ~ wiener(threshold[i], ndt[i], inv_logit(b[i] + (omega[i] * sv[cue[i, t]])), drift[cue[i, t]]); // check weiner; it only gives rt to upper boundary
      } else { // drift to withdrawal
        RT[i,t] ~ wiener(threshold[i], ndt[i], 1 - inv_logit(b[i] + (omega[i] * sv[cue[i, t]])), -drift[cue[i, t]]); // check weiner; it only gives rt to upper boundary
      }
...
generated quantities {
  ...
  mu_b          = Phi_approx(mu_pr[5]);
  mu_omega      = exp(mu_pr[6]);
  ...
  { // local section, this saves time and space
    ...
    for (i in 1:N) {
      ...
      for (t in 1:Tsubj[i]) {
        ...
        // Drift diffusion process
        if (pressed[i, t]) {
          log_lik[i] += wiener_lpdf(RT[i, t] | threshold[i], ndt[i], inv_logit(b[i] + (omega[i] * sv[cue[i, t]])), drift[cue[i, t]]);
        } else {
          log_lik[i] += wiener_lpdf(RT[i, t] | threshold[i], ndt[i], 1 - inv_logit(b[i] + (omega[i] * sv[cue[i, t]])), -drift[cue[i, t]]);
        }
        tmp_os          = wiener_rng(threshold[i], ndt[i], inv_logit(b[i] + (omega[i] * sv[cue[i, t]])), drift[cue[i, t]]);
