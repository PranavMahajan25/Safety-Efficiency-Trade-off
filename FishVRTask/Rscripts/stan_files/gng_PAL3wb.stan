...
parameters {
  ...
  vector[N] omega_pr;       // PIT arbitration param
...
transformed parameters {
  ...
  for (i in 1:N) {
    ...
    omega[i] = Phi_approx(mu_pr[3] + sigma[3] * omega_pr[i]);
  }
  b = mu_pr[4] + sigma[4] * b_pr;
}
...
// gng_m4: RW(rew/pun) + noise + bias + pi model (M5 in Cavanagh et al 2013 J Neuro)
  ...
  for (i in 1:N) {
    ...
    for (t in 1:Tsubj[i]) {
      wv_g[cue[i, t]]  = (1 - omega[i]) * qv_g[cue[i, t]] + b[i];
