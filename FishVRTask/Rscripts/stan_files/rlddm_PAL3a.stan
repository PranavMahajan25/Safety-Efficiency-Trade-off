...
transformed parameters {
  ...
  for (i in 1:N) {
    ...
    omega[i] = Phi_approx(mu_pr[6] + sigma[6] * omega_pr[i]);
  }
...
model {
  ...
  for (i in 1:N) {
    ...
    for (t in 1:Tsubj[i]) {
      ...
      wv_g[cue[i, t]]  = (1-omega[i]) * qv_g[cue[i, t]];
      wv_ng[cue[i, t]] = (1-omega[i]) * qv_ng[cue[i, t]] + omega[i] * (-sv[cue[i, t]]); 
      // Drift diffusion process
      drift[cue[i, t]] = invtemp[i] * (wv_g[cue[i, t]] - wv_ng[cue[i, t]]); 
    
      if (pressed[i, t]) { // drift to approach
        RT[i,t] ~ wiener(threshold[i], ndt[i], b[i], drift[cue[i, t]]); // check weiner; it only gives rt to upper boundary
      } else { // drift to withdrawal
        RT[i,t] ~ wiener(threshold[i], ndt[i], 1-b[i], -drift[cue[i, t]]); // check weiner; it only gives rt to upper boundary
      }
  ...
  { // local section, this saves time and space
    ...
    for (i in 1:N) {
      ...
      for (t in 1:Tsubj[i]) {
        ...
        // Drift diffusion process
        if (pressed[i, t]) {
          log_lik[i] += wiener_lpdf(RT[i, t] | threshold[i], ndt[i], b[i], drift[cue[i, t]]);
        } else {
          log_lik[i] += wiener_lpdf(RT[i, t] | threshold[i], ndt[i], 1-b[i], -drift[cue[i, t]]);
        }
        tmp_os          = wiener_rng(threshold[i], ndt[i], b[i], drift[cue[i, t]]);
