...
transformed parameters {
  ...
  kappa = exp(mu_pr[6] + sigma[6] * kappa_pr); // exp?
  assoc0 = exp(mu_pr[7] + sigma[7] * assoc0_pr); // needs to be positive, so exp?
}
...
// gng_m4: RW(rew/pun) + noise + bias + pi model (M5 in Cavanagh et al 2013 J Neuro)
  ...
  sigma[4:7]   ~ normal(0, 0.2);
  sigma[7] ~ normal(0, 1.0);
  ...
  for (i in 1:N) {
    ...
    for (t in 1:Tsubj[i]) {
      ...
      if (pressed[i, t]) { // update go value
        ...
        absRPE = abs(rho[i] * outcome[i, t] - qv_g[cue[i, t]]);
      } else { // update no-go value
        qv_ng[cue[i, t]] += ep[i] * (rho[i] * outcome[i, t] - qv_ng[cue[i, t]]);
        absRPE = abs(rho[i] * outcome[i, t] - qv_ng[cue[i, t]]);
      }
